...
class SMRC_View(object):
    ...
    def __init__(self, master):
        ...
        master.title("  Smart Model-Railway-Cam 2.0")
        self.cameraState=CamState.off
        self.initOK= True

        print('Initializing SMRC Controller...')
        try:
            self.smrcc= SMRC_Contr(self)
        except Exception as e:
            self.initOK= False
            print('   ERROR: SMRC Controller could not be initialized properly.')
            print(e)  
        else:
    ...
    def runCamera(self):
        ...
        while self.cameraState == CamState.on:
            pil_image = self.smrcc.takePhoto()
            #Run inference on MediaPipe object detector
            detection_result= self.smrcc.predict_image(pil_image)
            #print(detection_result.detections)
            pil_image= self.smrcc.process_inference_results(pil_image, detection_result, self.showDetectRegions.get())
